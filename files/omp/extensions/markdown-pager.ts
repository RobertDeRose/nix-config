...
export default function (pi: ExtensionAPI) {
  ...
  function installPagerFooter(ctx: any, renderFn: () => string[]) {
    const previous = ctx.ui.getFooter ? ctx.ui.getFooter() : undefined;
    ctx.ui.setFooter(() => ({
      invalidate() {},
      render() { return renderFn(); },
    }));
    return () => {
      if (previous) ctx.ui.setFooter(previous);
      else hideFooter(ctx);
    };
  }
  ...
  async function openPager(ctx: any, markdown: string, options: { shutdownOnClose?: boolean } = {}) {
    ...
    try {
      ...
      const Markdown = pi.pi?.Markdown as MarkdownCtor | undefined;
      if (!Markdown) {
        ctx.ui.notify("Markdown pager unavailable: OMP did not expose Markdown", "error");
        return;
      }
      const mdTheme = pi.pi?.getMarkdownTheme?.() ?? ctx.ui.theme;
